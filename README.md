...
ya que necesita persistir su valor entre llamadas sucesivas a `debounceFSM_update()`
— una variable local se reiniciaría en cada vuelta del loop, perdiendo el estado de
la MEF. Al estar todo el código en un único `main.c`, no es necesario declararla
`static` (no hay otro archivo del cual ocultarla); esto cambia en la versión
modularizada (Práctica 4 - Punto 2).
